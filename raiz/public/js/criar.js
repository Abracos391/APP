...
document.addEventListener('DOMContentLoaded', function() {
    ...
    imageForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (response.ok) {
                // Sucesso
                currentImageId = data.imagem_id;
                currentCredits = data.creditos_restantes;
                creditsCount.textContent = currentCredits;

                // Mostrar imagem gerada
                const imageUrl = data.caminho_arquivo;
    ...
    buyCreditsBtn.addEventListener('click', async function() {
        if (!verificarAutenticacao()) return;
        ...
        const creditosComprados = 10; // Simulação de compra de 10 créditos

        this.disabled = true;
        this.textContent = 'Comprando...';

        try {
            const response = await fetch(`${API_URL}/pagamento/simular-compra`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
            ...
            if (response.ok) {
                alert(data.mensagem);
                // Recarregar perfil para atualizar a contagem de créditos
                await carregarPerfil();
            } else {
                alert(data.erro || 'Erro ao simular a compra de créditos.');
            }
        } catch (error) {
            console.error('Erro na simulação de compra:', error);
            alert('Erro ao conectar com o servidor para simular a compra.');
        } finally {
            this.disabled = false;
            this.textContent = 'Comprar mais';
        }
    });
