...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (token) {
        // Se houver token, tentar carregar o perfil para validar
        fetch(`${API_URL}/api/auth/perfil`, {
            ...
            if (response.ok) {
                // Token válido, redirecionar para a página de criação
                window.location.href = '/app';
            } else {
                // Token inválido, remover e continuar no cadastro
                localStorage.removeItem('token');
            }
        }).catch(error => {
            console.error('Erro ao verificar token:', error);
            // Erro de conexão, continuar no cadastro
        });
    }
        ...
        cadastroForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    alert(`Cadastro realizado com sucesso! Você ganhou ${data.usuario.creditos_disponiveis} créditos gratuitos.`);
                   ...
                   // Redirecionar para a página principal
                    window.location.href = '/app'; // Redireciona para a página de criação
                } else {
                    // Erro no cadastro
