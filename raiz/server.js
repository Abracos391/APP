...
// Página de cadastro (rota principal e explícita)
...
// Rota catch-all para 404 (exceto API)
app.get('*', (req, res) => {
    ...
    // Qualquer outra rota não mapeada (404)
    res.status(404).send('Página não encontrada');
});
...
// A conexão com o banco de dados é inicializada no require('./backend/config/database')
// A inicialização do banco de dados (criação de tabelas) é feita dentro do database.js.
