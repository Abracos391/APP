...
// POST /api/pagamento/simular-compra - Simular compra e concessão de créditos
router.post('/simular-compra', (req, res) => {
    const { usuario_id, creditos_comprados } = req.body;

    if (!usuario_id || !creditos_comprados) {
        return res.status(400).json({ erro: 'usuario_id e creditos_comprados são obrigatórios' });
    }

    db.run(
        'UPDATE usuarios SET creditos_disponiveis = creditos_disponiveis + ? WHERE id = ?',
        [creditos_comprados, usuario_id],
        function(err) {
            if (err) {
                console.error('Erro ao conceder créditos:', err);
                return res.status(500).json({ erro: 'Erro ao conceder créditos' });
            }
            ...
            // Simular log de pagamento
            db.run(
                'INSERT INTO pagamentos (usuario_id, plano, valor, status, referencia_externa) VALUES (?, ?, ?, ?, ?)',
                [usuario_id, 'Simulação', 10.00, 'confirmado', `SIMULACAO_${Date.now()}`],
                (err) => {
                    if (err) console.error('Erro ao logar pagamento simulado:', err);
                }
            );

            res.json({ mensagem: `Compra simulada com sucesso. ${creditos_comprados} créditos adicionados.` });
        }
    );
});
